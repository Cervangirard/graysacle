...
#' @return tagList
#' @export
graysacle_one_project_img <- function(src_to_img, title, description ,dark = FALSE){
...
#' Another formatting project with image
#'
...
#' @return tagList
#' @export
graysacle_projects_img <- function(img, title, description, img_first = TRUE){
  if(img_first){
...
#' @param description description
#' @param dark if TRUE black background otherwise white
#' @param ... html code (like ggplot or table)
#'
#' @return tagList
#' @export
graysacle_one_project <- function(..., title, description ,dark = FALSE){

  if(dark){text_color <- 'text-black'}else{text_color <- 'text-white'}
  tagList(
    tags$div(class = "row align-items-center no-gutters mb-4 mb-lg-5",
             tags$div(class = "col-xl-8 col-lg-7",
                      tags$div(class = "mb-3 mb-lg-0", ...)
             ),
             tags$div(class = "col-xl-4 col-lg-5",
                      tags$div(class = "featured-text text-center text-lg-left",
                               h4(title,
...
#' @param title title
#' @param description description
#' @param first if TRUE, your html code should be on the left side
#' @param ... html code (like ggplot or table)
#'
#' @return tagList
#' @export
graysacle_projects <- function(..., title, description, first = TRUE){
  if(first){
    class <- "col-lg-6 order-lg-first"
    align <- "text-lg-right"
    line <- "mb-0 mr-0"
  ...
  tagList(
    tags$div(class = "row justify-content-center no-gutters",
             tags$div(class = "col-lg-6", tags$div(...)),
             tags$div(class = class,
                      tags$div(class = "bg-black text-center h-100 project",
                               tags$div(class = "d-flex h-100",
                                        tags$div(class = paste("project-text w-100 my-auto text-center", align),
